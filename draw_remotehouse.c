...
void DrawRemotehouse() {
    ...
    if (!isModelLoaded) {
        model = LoadModel("models/remotehouse.obj"); // Load the model
        Texture2D texture = LoadTexture("models/remotehouse.png"); // Load the texture
    ...
    Vector3 modelPosition = {1000.0f, 0.0f, 1000.0f }; // Adjust position as needed
